...
function validProjectPeriod(p){const a=p.projectStart??'',b=p.projectEnd??'';return (a===''&&b==='')||(validDate(a)&&validDate(b)&&a<=b);}
function validate(data){
 ...
 for(const p of data.projects){if(!p||!str(p.id)||!p.id||ids.has(p.id)||!str(p.name)||!p.name.trim()||!str(p.number)||!str(p.contact)||!str(p.notes)||typeof p.pinned!=='boolean'||!priority(p.priority)||!validSchedule(p)||!validProjectPeriod(p))return false;ids.add(p.id);}
 const tids=new Set();
...
const api={iso,parse,today,range,shift,nextTask,sortTasks,validate,validValue,validProjectPeriod};root.BCore=api;if(typeof module!=='undefined')module.exports=api;
